public struct MediumConfigurationFormatter {
	...
	CONFIGURATION
		...
		\(format(collection: config.articleReferences.map(\.description), label: "Articles:", isMultiline: true))
		...
		\(format(collection: config.tags, label: "Hubs:"))
		\(format(string: config.directory.path().removingPercentEncoding!, label: "Directory:"))
